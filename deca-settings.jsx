        ...
        <tbody>
          {muntins.map(m => (
            <tr key={m.id}>
              ...
              <td className="mono">{(+m.width || 0).toFixed(1)}</td>
        ...
        <tbody>
          {glass.map(g => (
            <tr key={g.id}>
              ...
              <td>
                <span className="mono" style={{ color: uValueColor(g.uValue || 0) }}>{g.uValue || 0}</span>
                <div className="u-value-bar">
                  <div className="u-value-bar-fill" style={{ width: `${uBarWidth(g.uValue || 0)}%`, background: uValueColor(g.uValue || 0) }} />
                </div>
              </td>
              <td className="mono">{g.stc || 0} dB</td>
              <td className="mono text-accent">${(+g.price || 0).toFixed(2)}</td>
              <td>
                <div className="cell-actions">
        ...
        <tbody>
          {accessories.map(a => (
            <tr key={a.id}>
              ...
              <td className="mono">{(a.overhang || 0) > 0 ? `±${a.overhang}mm` : "—"}</td>
              <td className="mono text-accent">${(+a.pricePerM || 0).toFixed(2)}</td>
              <td>
                <div className="cell-actions">
        ...
        <tbody>
          {screens.map(s => (
            <tr key={s.id}>
              ...
              <td className="mono text-accent">${(+s.price || 0).toFixed(2)}/m²</td>
              <td>
                <div className="cell-actions">
...
export default function App() {
  ...
  // ── API Layer ──
  const API_BASE = 'https://api.decacrm.com/configurator';
  const api = {
    get: async (path) => { const r = await fetch(`${API_BASE}${path}`); if (!r.ok) throw new Error(`API ${r.status}`); return r.json(); },
    post: async (path, body) => { const r = await fetch(`${API_BASE}${path}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); if (!r.ok) throw new Error(`API ${r.status}`); return r.json(); },
    put: async (path, body) => { const r = await fetch(`${API_BASE}${path}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); if (!r.ok) throw new Error(`API ${r.status}`); return r.json(); },
    del: async (path) => { const r = await fetch(`${API_BASE}${path}`, { method: 'DELETE' }); if (!r.ok) throw new Error(`API ${r.status}`); return r.json(); },
  };
    ...
    (async () => {
      ...
      try {
        ...
        if (settings && settings.profiles) {
          ...
          const apiColors = {
            profile: (settings.colors || []).filter(c => c.category === 'profile').map(c => ({ id: c.id, name: c.name, ral: c.ral, hex: c.hex })),
            handle: (settings.colors || []).filter(c => c.category === 'handle').map(c => ({ id: c.id, name: c.name, hex: c.hex })),
          };
          const apiProfiles = (settings.profiles || []).map(p => ({
            id: p.id, name: p.name, code: p.code,
            frame: { section: +p.frame_section, int: +p.frame_int, bead: +p.frame_bead, depth: +p.frame_depth },
            sash: { ext: +p.sash_ext, int: +p.sash_int, bead: +p.sash_bead, overlap: +p.sash_overlap },
            mullion: { total: +p.mullion_total, visible: +p.mullion_visible, rebate: +p.mullion_rebate },
            glass: { depth: +p.glass_depth },
          }));
          const apiGlass = (settings.glass || []).map(g => ({
            id: g.id, name: g.name, type: g.type, layers: g.layers || `${g.type === 'triple' ? '4-12-4-12-4' : '4-16-4'}`,
            gasType: g.gas_fill || 'air', lowE: g.low_e, uValue: +(g.u_value || 0), stc: g.stc, price: +(g.price_sqm || 0), desc: '',
          }));
          const apiMuntins = (settings.muntins || []).map(m => ({
            id: m.id, name: m.name, width: +m.width_mm, widthLabel: m.width_label, color: m.color, colorName: m.color_name,
          }));
          const apiHardware = (settings.hardware || []).map(h => ({
            id: h.id, name: h.name, category: h.category, brand: h.brand, finish: h.finish, price: +h.price,
          }));
          const apiAccessories = (settings.accessories || []).map(a => {
            const specs = (typeof a.specs === 'string' ? JSON.parse(a.specs) : a.specs) || {};
            return {
              id: a.id, name: a.name, type: a.type, material: specs.material || 'aluminum',
              pricePerM: +(a.price || 0), overhang: +(specs.width_mm || specs.overhang || 0), desc: specs.desc || '',
            };
          });
          const apiScreens = (settings.screens || []).map(s => ({
            id: s.id, name: s.name, type: s.type, mesh: s.mesh, frameColor: s.frame_color, price: +s.price,
          }));
          // Constraints: rebuild grouped format from API rows
          const apiConstraints = { _groups: [] };
          (settings.constraints || []).forEach(g => {
            const items = (typeof g.items === 'string' ? JSON.parse(g.items) : g.items) || [];
            apiConstraints._groups.push({ id: g.id, name: g.group_name, icon: g.group_icon, items });
            items.forEach(it => { apiConstraints[it.key] = it.value; });
          });
          const apiUsers = (settings.users || []).map(u => ({
            id: u.id, name: u.name, email: u.email, phone: u.phone, role: u.role,
            avatar: u.avatar, sellerCode: u.seller_code, active: u.active,
            permissions: u.permissions || {},
          }));

          setProfiles(apiProfiles);
          setColors(apiColors);
          setMuntins(apiMuntins);
          setGlass(apiGlass);
          setHardware(apiHardware);
          ...
          try {
            const apiCustomers = await api.get('/customers');
            if (Array.isArray(apiCustomers)) {
              // Load full data for each customer
              const full = [];
              for (const c of apiCustomers) {
                try {
                  const cf = await api.get(`/customers/${c.id}/full`);
                  full.push({
                    id: cf.id, firstName: cf.first_name, lastName: cf.last_name,
                    email: cf.email, phone: cf.phone, address: cf.address,
                    type: cf.type, company: cf.company, notes: cf.notes,
                    projects: (cf.projects || []).map(p => ({
                      id: p.id, name: p.name, address: p.address, status: p.status,
                      createdAt: p.created_at?.slice(0, 10),
                      constructions: [],
                      quotes: (p.quotes || []).map(q => ({
                        id: q.id, quoteNumber: q.quote_number, name: q.name,
                        status: q.status, amount: +q.amount, date: q.created_at?.slice(0, 10),
                        startedAt: q.started_at, invoices: q.invoices || [], items: [],
                        constructions: (q.constructions || []).map(cn => ({
                          id: cn.id, w: cn.width_mm, h: cn.height_mm, type: cn.type,
                          profileId: cn.profile_id, tree: cn.tree, defaults: cn.defaults,
                          positions: (cn.positions || []).map(pos => ({
                            idx: pos.idx, desc: pos.description,
                            posW: pos.pos_w, posH: pos.pos_h, posGlass: pos.pos_glass_id,
                          })),
                        })),
                      })),
                    })),
                  });
                } catch (e) { full.push({ ...c, projects: [] }); }
              }
              setCustomers(full);
            }
          } catch (e) { /* customers load failed, keep defaults */ }
        }
      } catch (e) {
        console.log('API unavailable, falling back to local storage');
      }

      // 2. Fallback: local storage (if API failed)
      if (!fromApi) {
        try {
  ...
  const saveQuoteToProject = async (custId, projId, quote) => {
    setCustomers(prev => prev.map(c => c.id === custId ? {
      ...c, projects: c.projects.map(p => p.id === projId ? {
    ...
    if (apiConnected) {
      try {
        const apiQuote = await api.post('/quotes', {
          project_id: projId, quote_number: quote.quoteNumber || quote.number || '',
          name: quote.name, description: quote.name, status: quote.status || 'draft',
          amount: quote.amount || 0, started_at: quote.startedAt,
        });
        // Update local quote with server ID
        if (apiQuote?.id) {
          setCustomers(prev => prev.map(c => c.id === custId ? {
            ...c, projects: c.projects.map(p => p.id === projId ? {
              ...p, quotes: p.quotes.map(q => q.id === quote.id ? { ...q, _apiId: apiQuote.id } : q)
            } : p)
          } : c));
        }
      } catch (e) { console.warn('API quote save failed:', e); }
    }
  };

  const deleteQuoteFromProject = async (custId, projId, quoteId) => {
    // Find API ID before deleting locally
    const cust = customers.find(c => c.id === custId);
    const proj = cust?.projects?.find(p => p.id === projId);
    const quote = proj?.quotes?.find(q => q.id === quoteId);
    setCustomers(prev => prev.map(c => c.id === custId ? {
      ...c, projects: c.projects.map(p => p.id === projId ? {
        ...p, quotes: (p.quotes || []).filter(q => q.id !== quoteId)
      } : p)
    } : c));
    if (apiConnected && (quote?._apiId || quoteId)) {
      try { await api.del(`/quotes/${quote?._apiId || quoteId}`); } catch (e) { console.warn('API quote delete failed:', e); }
    }
  };
      ...
      <div className="app-shell">
        ...
        <div className="topbar">
          ...
          <div className="topbar-right">
            <span title={apiConnected ? 'Connected to API' : 'Offline — using local storage'} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, color: apiConnected ? 'var(--green)' : 'var(--text-muted)', cursor: 'default' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: apiConnected ? 'var(--green)' : 'var(--text-muted)' }}></span>
